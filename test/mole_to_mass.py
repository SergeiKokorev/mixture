def mole_to_mass_fraction(y: list, M:list):
    denominator = sum([yi * Mi for yi, Mi in zip(y, M)])
    return [yi * Mi / denominator for yi, Mi in zip(y, M)]
...
if __name__ == "__main__":
    ...
    mass_fractions = mole_to_mass_fraction([fluid["mole_fraction"] for fluid in fluid_list], [fluid["mw"] for fluid in fluid_list])

    for mf, fluid in zip(mass_fractions, fluid_list):
        print(f"{fluid["name"]} : {fluid["symbol"]} : mass fraction : {mf}")
    print(sum(mass_fractions))
